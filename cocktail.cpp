...
// Cocktail 
// ======================= 
Cocktail::Cocktail(string n, vector<LigneIngredient> ingredients_list) 
    : name(n), ingredients(ingredients_list) {}

void Cocktail::display() {
    cout << " * " << name << ":" << endl;
    for (LigneIngredient &li : ingredients) {
        cout << " - " << li.name << " (" << li.quantity_needed << ")" << endl;
    }
}
...
bool Cocktail::checkIfCanBeDone(vector<Ingredient>& stock) {
    ...
    for (LigneIngredient& li : ingredients) {
        ...
        for (Ingredient& ing : stock) {
            if (ing.name == li.name && ing.quantity >= li.quantity_needed) {
                ingredient_ok = true;
            }
        }
        if (!ingredient_ok) {
            cout << " ⚠ Missing " << li.name << " to prepare " << name << endl;
            can_be_cooked = false;
        }
    }
